...
class Player(pomice.Player):
    ...
    async def do_next(self) -> None:
        ...
        # Call the controller and check if one exist
...
class MusicPlayer(commands.Cog):
    ...
    def required(self, ctx: commands.Context):
        """ Return required votes based on amount of members in a channel """
    ...
    def is_privileged(self, ctx: commands.Context):
        """ Check if the user is an admin or DJ """
    ...
    async def pomice_join(self, ctx: commands.Context, *, channel: discord.VoiceChannel = None) -> None:
        if not channel:
            channel = getattr(ctx.author.voice, "channel", None)
            if not channel:
                return await ctx.send(
                    "Please enter a voice channel first",
                )
        await ctx.author.voice.channel.connect(cls=Player)
        player: Player = ctx.voice_client
        
        # Set the player context so we can use it to send messages
        await player.set_context(ctx = ctx)
        await ctx.send(f"Joined the voice channel : {channel.name}")

    @commands.hybrid_command()
    async def pomice_leave(self, ctx: commands.Context):
        if not (player := ctx.voice_client):
            return await ctx.send(
                "The bot is currently not in a channel",
                delete_after = 5,
            )
    ...
    @commands.hybrid_command()
    async def pomice_play(self, ctx: commands.Context, *, search: str) -> None:
        """ play a song """
        # Check if the player is in the channel before we play anything
        if not (player := ctx.voice_client):
            await ctx.author.voice.channel.connect(cls=Player)
            player: Player = ctx.voice_client
            await player.set_context(ctx=ctx)
        ...
        if not results:
            return await ctx.send("No results were found for that search term", delete_after=5)
    ...
    @commands.hybrid_command()
    async def pomice_pause(self, ctx: commands.Context):
        ...
        if len(player.pause_votes) >= required:
            await ctx.send("Vote to pause passed, Pausing player!", delete_after = 5)
            player.pause_votes.clear()
    ...
    @commands.hybrid_command()
    async def pomice_resume(self, ctx: commands.Context):
    ...
    @commands.hybrid_command()
    async def pomice_skip(self, ctx: commands.Context):
        ...
        if self.is_privileged(ctx):
            await ctx.send("An admin or DJ has skpped the song", delete_after=5)
    ...
    @commands.hybrid_command()
    async def pomice_stop(self, ctx: commands.Context):
    ...
    @commands.hybrid_command()
    async def pomice_shuffle(self, ctx: commands.Context):
    ...
    async def volume(self, ctx: commands.Context, *, vol: int):
        ...
        await ctx.send(f"Set the volume to **{vol}**", delete_after=6)
